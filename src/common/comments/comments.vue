  ...
  <div id="comments">
    ...
    <ul class="comments-wrap">
      <li
        ...
        <ul class="comments-children" v-if="child >= 0">
          <li
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        placeholder: '您的每一条评论是作者最大的帮助和鼓励',
        content: '',
        showEmoji: false,
        count: 0,
        userName: '',
    ...
    created() {
      ...
      if (this.commentsInfo.userName) {
        this.userName = this.commentsInfo.userName
    ...
    methods: {
      ...
      getName(comments) {
        return comments.userName + (comments.isAuthor !== '1' ? '' : '（博主）')
      },
      getAvatar(comments) {
        if (comments.gravatar == null || comments.gravatar == '') {
          return this.avatar;
        } else {
          return comments.gravatar;
        }
      },
      init() {
        this.count = 0
        ...
        if (this.isAdminWrap) {
          this.getComments(this.id)
            .then((data) => {
              ...
              this.childLength = data.list.children
              this.child = data.list.children.length
            })
            .catch(() => {
            })
        } else {
      ...
      check() {
        if (this.userName === '' && !this.isAdminWrap) {
          this.$toast('请填写您的昵称', 'error')
          return
        }
      ...
      reply(comments) {
        this.replyId = comments.commentId
        this.replyName = `回复 ${comments.userName} `
